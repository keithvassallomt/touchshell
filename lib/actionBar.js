...
// Touchshell-specific stage action name used by add_action_full.
...
export class ActionBar {
    ...
    _setup() {
        ...
        this._driver = new WorkspaceSwitchDriver({
            ...
            // Touchup-equivalent feel: a workspace step commits in
            // ~62% of monitor width instead of 100%.
            distanceFactor: 0.62,
        });
    ...
    _evaluate() {
        ...
        // Pill drawn only when not exception, not fullscreen, not in overview.
        const showPill =
            !this._exceptionMatch && !this._fullscreen && !this._inOverview;
        this._setPillVisible(showPill);

        // Touch zone: live unless app is on the exception list. Fullscreen
        // and overview gating happens in _shouldHandleSequence (overview)
        // or is naturally fine (fullscreen apps still want workspace switch).
        // Nothing else to do here for the touch side.

        // Fill class depends on fullscreen / exception state too.
        this._refreshThemeClasses();

        // Brightness re-evaluation.
        this._scheduleSample();
    }

    _setupWindowTracking() {
        // Re-evaluate on size changes (maximize/unmaximize/resize).
        this._globalSignals.push([
            global.window_manager,
            global.window_manager.connect('size-change',
                () => this._scheduleNearCheck()),
        ]);
        // New windows mapping in.
        this._globalSignals.push([
            global.display,
    ...
    _retrackWindows() {
        // Disconnect all currently-tracked windows.
        for (const [win, ids] of this._trackedWindows) {
            for (const id of ids) {
                try { win.disconnect(id); } catch (_) {}
            }
    ...
    _evaluateWindowNear() {
        ...
        this._setWindowNear(near);
    }

    _setWindowNear(near) {
        if (this._isWindowNear === near) return;
    ...
    // Apply CSS classes that depend on theme darkness + adjacency state.
    // - touchshell-actionbar--filled    → opaque background so the bar
    //   blends with the touching window's surrounding chrome instead of
    //   exposing the desktop colour. Toggled by _isWindowNear; in
    //   fullscreen we keep it transparent so the fullscreen content
    //   isn't framed by a 22px strip on top.
    // - touchshell-actionbar--theme-dark → stylesheet picks the dark
    //   variant of bar background and pill default colour.
    _refreshThemeClasses() {
        ...
        const fill = this._isWindowNear && !this._fullscreen
            && !this._exceptionMatch;
        if (fill) {
            this._actor.add_style_class_name('touchshell-actionbar--filled');
        } else {
            this._actor.remove_style_class_name(
                'touchshell-actionbar--filled');
        }

        if (this._isDarkTheme) {
            this._actor.add_style_class_name(
