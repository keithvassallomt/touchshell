...
export class ActivationBinding {
    constructor(settings, tabletMode, key, { onEnable, onDisable }) {
        ...
        this._settingsId = settings.connect(`changed::${key}`, () => this._sync());
        this._tabletId = tabletMode.connect('changed', () => this._sync());
        this._sync();
    }
    ...
    destroy() {
        ...
        if (this._tabletId) {
            this._tabletMode.disconnect(this._tabletId);
            this._tabletId = 0;
        }
        this._settings = null;
