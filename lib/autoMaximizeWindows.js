...
export class AutoMaximizeWindows {
    ...
    _uninstallOpacityClamp(actor) {
        if (!actor._touchshellOpacityClamp) return;
        try { actor.disconnect(actor._touchshellOpacityClamp); }
        catch (_) {}
    ...
    // Re-check exemption once we're back from synchronous window-
    // created handling. Two reasons we may need to retry:
    //   1. Shell.WindowTracker only associates a window with its
    //      .desktop after focus/title/wm-class signals settle. For
    //      windows summoned over a fullscreen app, that resolution
    //      can be slower than a single idle tick — _appIds() comes
    //      back empty and _isExempt() reports false even though the
    //      app *is* on the exception list. Retry up to 5 times,
    //      100ms apart, before giving up and treating it as a
    //      regular GTK4 max-bounce candidate.
    //   2. We hold the actor invisible across the whole bounce so
    //      the user never sees the maximized intermediate state.
    //      Reveal happens AFTER the final move (centered popups via
    //      _centerInWorkArea, otherwise immediately).
    _idleRecheck(win, savedRectIsTrustworthy, attempt) {
        ...
        try {
            ...
            // Shell.WindowTracker only associates a window with its
            // .desktop after focus/title/wm-class signals settle.
            // For windows summoned over a fullscreen app, that
            // resolution can be slower than a single idle tick —
            // _appIds() comes back empty and _isExempt() reports
            // false even though the app *is* on the exception list.
            // Retry up to 5 times, 100ms apart.
    ...
    _centerInWorkArea(win) {
        ...
        const cleanupWaitForUnmax = () => {
            if (sizeSignalId) {
                try { win.disconnect(sizeSignalId); } catch (_) {}
                sizeSignalId = 0;
            }
            if (timeoutId) {
        ...
        const cleanupFightBack = () => {
            if (fightSignalId) {
                try { win.disconnect(fightSignalId); } catch (_) {}
                fightSignalId = 0;
            }
            if (graceTimeoutId) {
