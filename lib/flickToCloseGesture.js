...
const DND_TIMEOUT_MS = 500;

// Press-feedback visual: a quiet "I see you" cue on touch-begin so the
// user knows the thumbnail is reacting before drag's full transform
// kicks in.
// Bumped from a near-imperceptible 0.97/240 to values that are
// clearly visible — the previous tuning was too subtle to read on a
// touchscreen at typical preview size.
const PRESS_SCALE = 0.92;
const PRESS_OPACITY = 200;
const PRESS_DOWN_MS = 80;
const PRESS_UP_MS = 120;
// Wait this long before showing the press visual. Quick taps (to
// activate a window) should not flash the press effect, and tapping
// then releasing before the dnd timeout shouldn't leave a stuck
// visual. Aligned roughly with iPad's "press-and-hold" feel.
...
export class FlickToCloseGesture {
    constructor(settings, tabletMode) {
        ...
        this._activationChangedId = this._settings.connect(
            'changed::flick-to-close-activation',
            () => this._syncEnabled()
        );
    ...
    _syncEnabled() {
        const activation = this._settings.get_string('flick-to-close-activation');
        const want =
            activation === 'always' ||
    ...
    _patchPreview(preview) {
        ...
        if (dnd && 'timeout_threshold' in dnd) {
            ...
            dnd.timeout_threshold = DND_TIMEOUT_MS;
        }
    ...
    _pressOff(preview, info) {
        ...
        if (info.dragInProgress || info.firedFlick)
            return;
        // Quick tap that released before the press-delay fired — nothing
        // to revert. Avoids stuck-effect when the user taps faster than
        // PRESS_DELAY_MS.
        if (!info.pressShown)
            return;
        info.pressShown = false;
