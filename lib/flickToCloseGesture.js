...
const DEBUG = false;
const LOG = (...a) => { if (DEBUG) console.log('[touchshell:flick]', ...a); };
...
export class FlickToCloseGesture {
    ...
    _unpatchPreview(preview, info) {
        ...
        if (info.capturedId)
            preview.disconnect(info.capturedId);
        if (info.dragBeginId && preview._draggable)
            preview._draggable.disconnect(info.dragBeginId);
        if (info.dragEndId && preview._draggable)
            preview._draggable.disconnect(info.dragEndId);
        ...
        if (info.destroyId)
            preview.disconnect(info.destroyId);
    }
}
