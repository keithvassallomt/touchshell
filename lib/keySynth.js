...
export class KeySynth {
    ...
    _pressSuper(keyval) {
        this._press(Clutter.KEY_Super_L);
        this._press(keyval);
        this._release(keyval);
        this._release(Clutter.KEY_Super_L);
    }

    // Native Mutter tiling shortcuts (toggle-tiled-left/right).
    tileLeft() { this._pressSuper(Clutter.KEY_Left); }
    tileRight() { this._pressSuper(Clutter.KEY_Right); }
}
