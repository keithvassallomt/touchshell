...
export class OverviewGestureDriver {
    ...
    begin() {
        ...
        this._active = true;
        LOG('begin distance=', this._distance,
    ...
    // pixels per millisecond (negative = up). Verified via journal logs.
        ...
        if (velocityYPxPerMs < -FLICK_VELOCITY_PX_PER_MS) {
            ...
            commitReason = 'flick-down';
        } else {
            endProgress = traveledFromStart >= COMMIT_PROGRESS_FRACTION
                ? nextSnap
                : prevSnap;
            commitReason = 'position';
        }

        const remaining = Math.abs(endProgress - this._currentProgress);
        // For velocity-driven commits, scale duration to the actual lift
        // speed so a hard flick doesn't crawl through a 300ms close.
        // For position-driven commits, the user just released slowly —
        // a steady remaining*MAX_DURATION pace feels right.
        let duration;
        if (commitReason !== 'position' && Math.abs(velocityYPxPerMs) > 0) {
        ...
        LOG('end velocityY=', velocityYPxPerMs.toFixed(2),
            'traveled=', traveledFromStart.toFixed(3),
            'commit→', endProgress, '(', commitReason, ')',
