...
const DEBUG = false;
const LOG = (...a) => { if (DEBUG) console.log('[touchshell:overview-driver]', ...a); };
...
export class OverviewGestureDriver {
    constructor({ distanceFactor = 1.0 } = {}) {
        // Scales the per-step distance reported by confirmSwipe. < 1.0
        // means less finger travel per state step (snappier gesture);
        // 1.0 is 1:1 with the controller's reported distance (typically
        // monitor height). Touchup tunes this around 0.6 by default for
        // its bottom-edge gestures; we expose it per call site.
    ...
    // The driver itself only checks whether the overview API is available
    // and stable. Callers add their own context gating (e.g. bottom-edge
    // requires overview NOT visible; in-overview swipes require it IS).
    canStart() {
    ...
    // pixels per millisecond (negative = up). Verified via journal logs.
        ...
        const lower = this._snapPoints[0];
        const upper = this._snapPoints[this._snapPoints.length - 1];
        // "Next" / "prev" snap points relative to the gesture's start —
        // works for any integer or fractional initialProgress.
        const nextSnap = Math.min(upper, Math.ceil(this._initialProgress + 0.0001));
        const prevSnap = Math.max(lower, Math.floor(this._initialProgress - 0.0001));
        ...
        if (velocityYPxPerMs < -FLICK_VELOCITY_PX_PER_MS) {
            ...
            // Strong downward flick → commit backward regardless of position.
