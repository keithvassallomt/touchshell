...
class PanelAutoHide extends GObject.Object {
    ...
    // Make the panel reachable for a panel-menu anchor and arrange to
    // un-reach it once the menu closes. Two failure modes get patched
    // here:
    //   1. Auto-hide may have the panel translated off-screen — fixed
    //      by showInstant() snapping translation_y back to 0.
    //   2. A fullscreen window on the primary monitor causes
    //      layoutManager._updateFullscreen() to flip panelBox.visible
    //      = false (the chrome has trackFullscreen: true). With the
    //      panelBox invisible, the indicator inside it has no valid
    //      stage allocation and the menu's BoxPointer arrow lands at
    //      (0, 0) — the menu opens top-left instead of under its
    //      indicator.
    // We force visible=true around the menu open and restore on close
    // if the monitor is still in fullscreen at that point.
    prepareForMenu(menu) {
        this.showInstant();
        const panelBox = Main.layoutManager.panelBox;
        const wasHidden = !panelBox.visible;
        if (!wasHidden)
            return;
        panelBox.visible = true;
        // Track this connection so _teardown can drop it if the
        // extension is disabled while the menu is still open.
        const id = menu.connect('open-state-changed', (_m, open) => {
            if (open)
                return;
            menu.disconnect(id);
            this._menuPrepareSignal = null;
            if (Main.layoutManager.primaryMonitor.inFullscreen)
                panelBox.visible = false;
        });
        this._menuPrepareSignal = [menu, id];
    }
    ...
    _teardown() {
        ...
        if (this._menuPrepareSignal) {
            const [menu, id] = this._menuPrepareSignal;
            menu.disconnect(id);
            this._menuPrepareSignal = null;
        }
    ...
    _disconnectFocusWindow() {
        for (const id of this._focusWindowSignals) {
            this._focusWindow?.disconnect(id);
        }
        this._focusWindowSignals = [];
        this._focusWindow = null;
    }
