...
class PanelAutoHide extends GObject.Object {
    ...
    _updateStruts() {
        ...
        // the panel is currently off-screen via translation_y=-h and we
        // re-add the chrome with affectsStruts:true, the resulting
        // strut clamps to a 0-height band and contributes nothing. So
        // when struts are coming back we snap the panel to y=0 first.
        // The follow-up _showPanel() is a no-op because we're already
        // at the target.
        if (reserveSpace) {
            panelBox.remove_all_transitions();
            panelBox.translation_y = 0;
        ...
        // addChrome only QUEUES a region update; force it synchronously
        // so any geometry that runs immediately after this (e.g. the
        // tile rect for a snap that just unmaximized us) sees the new
        // work area. _updateRegions() is private but stable for years.
