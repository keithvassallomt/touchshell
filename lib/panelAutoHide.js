...
class PanelAutoHide extends GObject.Object {
    ...
    // MonitorConstraint(work-area=true). When our auto-hide removes the
    // panel's struts (steady state), the work area becomes the full monitor
    // and the banner lands at y=0 — under the panel whenever the panel is
    // showing. Compensate by translating the banner down by panel height.
    // During the overview the panel reserves struts again, so the work
    // area already excludes the panel and no offset is needed.
    _bannerOffsetWhenPanelShown() {
        return this._overviewOpen ? 0 : this._panelHeight();
    }
