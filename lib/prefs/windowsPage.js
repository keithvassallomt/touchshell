...
export function build(settings) {
    ...
    page.add(_tilingFlickGroup(settings));
    page.add(_panelAutoHideGroup(settings));

    return page;
}

function _tilingFlickGroup(settings) {
    const group = new Adw.PreferencesGroup({
        title: 'Flick to tile',
    });

    group.add(buildFeatureRow(
        'window-tiling-flick.webm', 'Flick to tile',
        settings, 'window-tiling-flick-activation',
        'While dragging a window by touch, flick it to snap: left or ' +
        'right tiles it to that half, up maximizes, down minimizes. Only ' +
        'a fast flick triggers it; a slow drag-and-drop leaves it where ' +
        'it lands. Mouse drags are unaffected.'));
