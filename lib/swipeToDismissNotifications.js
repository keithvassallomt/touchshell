...
export class SwipeToDismissNotifications {
    ...
    // ---- Banner (top-of-screen transient): swipe up ----
    ...
    _teardownBannerGesture() {
        ...
        if (this._bannerGesture && bin) {
            try {
                ...
                // Bin may have been replaced during shell teardown.
            }
        }
        this._bannerGesture = null;
    ...
    // ---- Date-menu list: swipe left ----
    //
    // NotificationMessage actors handle their own touch events (they
    // extend St.Button), so a CAPTURE gesture on the parent list
    // never sees them. Instead we attach a dedicated PanGesture per
    // message — same pattern touchup uses. Existing messages get
    // patched in onEnable; new ones get patched by wrapping
    // NotificationMessageGroup.prototype._addNotification, which
    // every group calls when a notification is added to its source.

    _setupListGesture() {
        // Catch new notifications added to any NotificationMessageGroup.
        ...
        // Patch all messages already in the date-menu list.
    ...
    _attachMessageGesture(message) {
        ...
        // Auto-cleanup when the message is destroyed (notification
        // dismissed, list cleared, etc.).
