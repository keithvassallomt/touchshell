...
const EDGE_MARGIN = 8;
// How far from the bottom-right corner the persistent collapsed FAB
// sits. iPad-ish thumb territory.
const CORNER_MARGIN = 24;
// Translucent at rest so the user knows it's there but it doesn't
// fight with what's underneath.
...
export class TextActionFab {
    constructor({ onAction, onPositionChanged }) {
        ...
        this._cutBtn = this._mkBarButton('Cut', 'cut');
        this._copyBtn = this._mkBarButton('Copy', 'copy');
        this._pasteBtn = this._mkBarButton('Paste', 'paste');
    ...
    // Hit-test whichever surface(s) we currently show.
    containsPoint(x, y) {
        const within = (actor) => {
    ...
    // Pop the action bar at (x, y). Hides the corner FAB while up.
    expandAt(x, y) {
        if (!this._bar) return;
        this._hideCornerImmediate();
    ...
    // Force everything off the screen (e.g. on disable).
    hide() {
        this._hideBar();
        this._hideCorner();
    }
    ...
    setEditable(canEdit) {
        ...
        for (const btn of [this._cutBtn, this._pasteBtn]) {
            if (!btn) continue;
            btn.reactive = editable;
            btn.opacity = editable ? 255 : 100;
        }
    }
    ...
    _onFabClicked() {
        // Tap on the always-visible corner FAB pops the bar at the
        // FAB's current position.
        const [fx, fy] = this._fab.get_position();
        this.expandAt(fx + FAB_SIZE / 2, fy + FAB_SIZE / 2);
    }

    _showCorner() {
        if (!this._fab || this._cornerVisible) return;
        console.log('[touchshell text-action] FAB _showCorner');
    ...
    _hideCorner() {
        if (!this._fab || !this._cornerVisible) return;
        console.log('[touchshell text-action] FAB _hideCorner');
        this._fab.remove_all_transitions();
        this._fab.ease({
            opacity: 0,
    ...
    // Snap the corner FAB out without animation — used when the bar
    // is opening at the same time as the corner needs to disappear.
    _hideCornerImmediate() {
        if (!this._fab) return;
        console.log('[touchshell text-action] FAB _hideCornerImmediate');
        this._fab.remove_all_transitions();
        this._fab.visible = false;
        this._fab.opacity = REST_OPACITY;
        this._fab.set_scale(1.0, 1.0);
        this._cornerVisible = false;
    }
