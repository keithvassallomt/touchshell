...
export class TopRightSwipeGesture {
    constructor(settings) {
        ...
        this._enableChangedId = this._settings.connect(
            'changed::enable-top-right-swipe',
            () => this._syncEnabled()
        );
        this._syncEnabled();
    }
    ...
    destroy() {
        ...
        if (this._enableChangedId) {
            this._settings.disconnect(this._enableChangedId);
            this._enableChangedId = 0;
        }
        this._settings = null;
    }

    _syncEnabled() {
        const want = this._settings.get_boolean('enable-top-right-swipe');
