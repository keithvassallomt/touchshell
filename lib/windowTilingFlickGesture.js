...
// ponytail: a missed touch event from exotic multi-touch could drift the
...
export class WindowTilingFlickGesture {
    ...
    _setup() {
        ...
        this._captureId = global.stage.connect(
            'captured-event', (_a, event) => this._onEvent(event));
        this._beginId = global.display.connect(
            'grab-op-begin', (_d, win, op) => this._onBegin(win, op));
        this._endId = global.display.connect(
    ...
    // ponytail: tile via the native Super+←/→ keybinding so it matches
    // drag-to-edge tiling exactly and inherits Mutter's tile state for
    // free. Ceiling: breaks if the user rebinds toggle-tiled-left/right;
