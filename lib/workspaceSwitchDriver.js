...
// Streams a 1:1 progress value into WorkspacesDisplay._switchWorkspace*
// so the workspace strip in the overview tracks the finger exactly the
// way the native 4-finger touchpad swipe does.
//
// confirmSwipe(distance, snapPoints, progress, cancelProgress):
//   distance      = WorkspacesDisplay.width (px per one workspace step)
...
export class WorkspaceSwitchDriver {
    ...
    canStart() {
        const wd = Main.overview?._overview?.controls?._workspacesDisplay;
        if (!wd)
            return false;
        ...
        if (Main.overview.animationInProgress)
            return false;
        if (typeof wd._switchWorkspaceBegin !== 'function')
            return false;
        if (typeof wd._switchWorkspaceUpdate !== 'function')
            return false;
        if (typeof wd._switchWorkspaceEnd !== 'function')
            return false;
